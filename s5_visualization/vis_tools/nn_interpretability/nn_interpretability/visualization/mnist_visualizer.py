...
class MnistVisualizer(LRPHeatmap):
    ...
    def display_images_with_probabilities_for_each_class(images, probabilities):
        for i in range(5):
            ...
            for j in range(2):
                ...
                plt.imshow(images[i * 2 + j], cmap="gray", vmin=0, vmax=1)
                plt.title(
                    "Digit {}, Probability {:.2f}".format(
                        i * 2 + j, probabilities[i * 2 + j]
                    )
                )
                plt.xticks([])
                plt.yticks([])
        ...
        for i in range(5):
            ...
            for j in range(2):
                ...
                plt.text(
                    0.12,
                    -0.1,
                    "Predicted {:d} times".format(int(times[i * 2 + j])),
    ...
    def cat_images(images, sx, sy):
        ...
        imgs_stack = np.hstack(
            image.squeeze(0).cpu().numpy().reshape(28, 28) for image in images
        )
        ...
        for i in range(len(images)):
            ...
            for j in range(top_k):
                plt.text(
                    0.5,
                    -0.5 - 0.5 * j,
                    "Class: {} \n{:.2f} %".format(cls[i][j], score[i][j] * 100),
                    size=12,
                    ha="center",
                    transform=ax.transAxes,
                )
