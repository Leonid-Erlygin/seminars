    ...
    def newlayer(self, layer, g):
        ...
        try:
            if layer.weight is not None:
                output_layer.weight = nn.Parameter(g(layer.weight))
        except AttributeError:
            pass

        try:
            if layer.bias is not None:
                output_layer.bias = nn.Parameter(g(layer.bias))
        except AttributeError:
            pass
                    ...
                    if isinstance(module, nn.Conv2d):
                        ...
                        if first_linear == True:
                            ...
                            newlayer.weight = nn.Parameter(
                                module.weight.reshape(n, channel_out, h_w[0], h_w[1])
                            )
                            first_linear = False
                        else:
                            ...
                            newlayer.weight = nn.Parameter(
                                module.weight.reshape(n, m, 1, 1)
                            )

                        if bias_switch is not False:
                            newlayer.bias = nn.Parameter(module.bias)

                        newlayers += [newlayer]

                    elif isinstance(module, nn.MaxPool2d) or isinstance(
                        module, nn.AvgPool2d
                    ):
                        h_w = conv_output_shape(
                            h_w,
...
def conv_output_shape(h_w, kernel_size=1, stride=1, pad=0, dilation=1):
    ...
    h = (h_w[0] + (2 * pad[0]) - (dilation[0] * (kernel_size[0] - 1)) - 1) // stride[
        0
    ] + 1
    w = (h_w[1] + (2 * pad[1]) - (dilation[1] * (kernel_size[1] - 1)) - 1) // stride[
        1
    ] + 1
