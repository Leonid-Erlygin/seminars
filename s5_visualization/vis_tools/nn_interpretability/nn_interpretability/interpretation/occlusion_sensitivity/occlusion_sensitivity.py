...
class OcclusionSensitivity(DecisionInterpreter):
    ...
    def __init__(
        ...
        if (
            self.input_size[0] % self.block_size[0] != 0
            or self.input_size[1] % self.block_size[1] != 0
        ):
    ...
    def _generate_occlused_input(self, x):
        ...
        for row in range(rows):
            for column in range(columns):
                ...
                new_x[0][0][
                    row * self.block_size[0] : (row + 1) * self.block_size[0],
                    column * self.block_size[1] : (column + 1) * self.block_size[1],
                ] = self.fill_value
                out.append(new_x)
