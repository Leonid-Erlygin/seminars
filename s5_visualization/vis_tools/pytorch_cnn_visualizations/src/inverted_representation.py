...
class InvertedRepresentation:
    ...
    def total_variation_norm(self, input_matrix, beta):
        ...
        total_variation = (
            ((to_check - one_bottom) ** 2 + (to_check - one_right) ** 2) ** (beta / 2)
        ).sum()
    ...
    def euclidian_loss(self, org_matrix, target_matrix):
        ...
        normalized_euclidian_distance = euclidian_distance / self.alpha_norm(
            org_matrix, 2
        )
    ...
    def generate_inverted_image_specific_layer(
        self, input_image, img_size, target_layer=3
    ):
        # Generate a random image which we will optimize
        opt_img = Variable(
            1e-1 * torch.randn(1, 3, img_size, img_size), requires_grad=True
        )
        ...
        input_image_layer_output = self.get_output_from_specific_layer(
            input_image, target_layer
        )
...
if __name__ == "__main__":
    ...
    inverted_representation.generate_inverted_image_specific_layer(
        prep_img, image_size, target_layer
    )
