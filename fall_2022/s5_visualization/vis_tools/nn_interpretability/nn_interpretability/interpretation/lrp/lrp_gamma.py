...
class LRPGamma(LRPBase):
    """
    Implements the decision-based interpretability method "LRP-gamma"
    ...
    def __init__(
        ...
        """
        super(LRPGamma, self).__init__(model, classes, preprocess)
    ...
    def interpret(self, x):
        ...
        for l in range(self.visualize_layer, L)[::-1]:
            ...
            if isinstance(self.LRP_layers[l], torch.nn.MaxPool2d) or isinstance(
                ...
                z = incr(
                    self.newlayer(self.LRP_layers[l], rho).forward(self.A[l])
                )  # step 1
                s = (self.Relevance[l + 1] / z).detach()  # step 2
                (z * s).sum().backward()
                c = self.A[l].grad  # step 3
