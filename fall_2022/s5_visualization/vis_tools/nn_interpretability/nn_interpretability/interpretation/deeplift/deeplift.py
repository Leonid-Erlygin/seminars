    ...
    def _register_backward_hook(self):
        ...
        def register_hook(layer):
            def rescale_hook(self, input, grad_out):
                ...
                far_zero_contrib_mask = (
                    (delta_x.abs() > near_zero_threshold).float().to(device)
                )
                with_multiplier = far_zero_contrib_mask * multiplier
                with_multiplier = with_multiplier.to(device)

                near_zero_contrib_mask = (
                    (delta_x.abs() <= near_zero_threshold).float().to(device)
                )
            ...
            def reveal_cancel_hook(self, input, grad_out):
                ...
                delta_y_plus = 0.5 * (
                    F.relu(ref_x + delta_x_plus) - F.relu(ref_x)
                ) + 0.5 * (
                    F.relu(ref_x + delta_x_plus + delta_x_minus)
                    - F.relu(ref_x + delta_x_minus)
                )

                delta_y_minus = 0.5 * (
                    F.relu(ref_x + delta_x_minus) - F.relu(ref_x)
                ) + 0.5 * (
                    F.relu(ref_x + delta_x_plus + delta_x_minus)
                    - F.relu(ref_x + delta_x_plus)
                )
            ...
            def linear_hook(self, input, grad_out):
                ...
                transpose_pos.weight = nn.Parameter(
                    ((transposed_weight > 0).float().to(device)) * transposed_weight
                )
                transpose_negative = nn.Linear(size[1], size[0]).to(device)
                transpose_negative.weight = nn.Parameter(
                    ((transposed_weight < 0).float().to(device)) * transposed_weight
                )
                ...
                neg_pos_result = (
                    transpose_negative.forward(neg_grad_out) * delta_x_minus
                )
                neg_neg_result = (
                    transpose_negative.forward(pos_grad_out) * delta_x_minus
                )
            ...
            def linear_conv_hook(self, input, grad_out):
                ...
                transpose_pos = nn.ConvTranspose2d(
                    self.out_channels,
                    self.in_channels,
                    self.kernel_size,
                    self.stride,
                ...
                transpose_pos.weight = nn.Parameter(
                    ((self.weight > 0).float().to(device))
                    * self.weight.detach().clone().to(device)
                ).to(device)

                transpose_negative = nn.ConvTranspose2d(
                    self.out_channels,
                    self.in_channels,
                    self.kernel_size,
                    self.stride,
                ...
                transpose_full = nn.ConvTranspose2d(
                    self.out_channels,
                    self.in_channels,
                    self.kernel_size,
                    self.stride,
                ...
                transpose_full.weight = nn.Parameter(
                    (self.weight.detach().clone().to(device))
                ).to(device)
            ...
            if self.rule != DeepLIFTRules.NoRule:
                if (
                    ...
                    and self.rule != DeepLIFTRules.Linear
                ):
                    if (
                        self.rule == DeepLIFTRules.LinearRescale
                        or self.rule == DeepLIFTRules.Rescale
                    ):
                        self.hooks.append(layer.register_backward_hook(rescale_hook))
                    else:
                        self.hooks.append(
                            layer.register_backward_hook(reveal_cancel_hook)
                        )
                elif isinstance(layer, torch.nn.Linear) and (
                    self.rule == DeepLIFTRules.LinearRescale
                    or self.rule == DeepLIFTRules.LinearRevealCancel
                    or self.rule == DeepLIFTRules.Linear
                ):
