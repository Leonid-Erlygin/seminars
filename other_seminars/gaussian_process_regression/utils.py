...
def plot_1D_function(func, X, axis_bounds=([0, 1], [-6.5, 16.5])):
    ...
    plt.plot(x_grid, func(torch.from_numpy(x_grid)).numpy(), label="Forrester function")
    plt.scatter(X.cpu().numpy(), func(X).cpu().numpy(), s=50, label="Initial sample")
...
def plot_acquisition(acquisition, X, y, X_candidate):
    ...
    plt.plot(
        x_grid,
        0.2 * factor * acqu_normalized - abs(min(lower)) - 0.25 * factor,
        "-r",
        lw=2,
        label="Acquisition",
    )
    ...
    plt.fill_between(x_grid, lower.ravel(), upper.ravel(), color=color, alpha=0.1)

    plt.ylim(min(lower) - 0.25 * factor, max(upper) + 0.05 * factor)
